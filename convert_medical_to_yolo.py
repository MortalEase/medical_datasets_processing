...
def read_mha_image(file_path):
    ...
    try:
        ...
        log_error(f"读取图像失败: {file_path}, 错误: {e}.")
        return None
...
def convert_medical_to_yolo(input_dir, output_dir, metadata_file):
    ...
    log_info("数据集统计:")
    log_info(f"总记录数: {len(metadata)}")
    log_info(f"唯一图像数: {metadata['img_name'].nunique()}")
    log_info(f"标签分布: {metadata['label'].value_counts()}")
    ...
    log_info("\n数据分析:")
    log_info(f"有结节图像: {len(images_with_nodules)} 张")
    log_info(f"无结节图像: {len(images_without_nodules)} 张")
    ...
    for img_name in tqdm(unique_images, desc="转换图像"):
        try:
                ...
                with open(label_output_path, 'w') as f:
                    for _, row in nodule_records.iterrows():
                        ...
                        if x < 0 or y < 0 or width <= 0 or height <= 0:
                            log_warn(f"图像 {img_name} 有无效的边界框坐标: x={x}, y={y}, w={width}, h={height}.")
                            continue
            ...
            log_error(f"处理图像 {img_name} 时出错: {e}.")
            error_count += 1
    
    log_info("\n转换完成.")
    log_info(f"成功转换: {converted_count} 张图像")
    log_info(f"转换失败: {error_count} 张图像")
    log_info(f"有结节图像(创建了标签文件): {nodule_images_count} 张")
    log_info(f"无结节图像(仅转换图像): {no_nodule_images_count} 张")
...
"""
    ...
    log_info(f"数据集配置文件已创建: {yaml_path}.")

def main():
    """主函数."""
    parser = argparse.ArgumentParser(description="医学图像数据集转 YOLO 格式")
    ...
    log_info("医学图像数据集转 YOLO 格式转换器 (修正版)")
    log_info("="*60)
    log_info(f"输入目录: {input_dir}")
    log_info(f"输出目录: {output_dir}")
    log_info(f"元数据文件: {metadata_file}")
    log_info("="*60)
    ...
    if not os.path.exists(metadata_file):
        log_error(f"元数据文件不存在: {metadata_file}.")
        return
